...
(simple), `InteractionMediationData` (simple with a treatment-by-mediator
interaction), `SerialMediationData`, `ParallelMediationData`,
`JointMediationData` (several mediators with treatment-by-mediator products),
and `BootstrapResult`. The three most common are introduced here; see
[Model Extraction](extraction.html) for the others.
