...
Pass engine-specific settings through `engine_args`:
...
med_rmi2 <- fit_mediation(
  ...
  engine = "regmedint",
  engine_args = list(
    m_cde = 1,          # reference mediator level (default 0, as with m_star)
...
`m_cde`, and `c_cond`; each replaces a value the adapter would otherwise derive
