...
fit_m2 <- lm(M2 ~ X + M1, data = data_lm_serial)
# Include every mediator in the outcome model, not only the last one
fit_y  <- lm(Y ~ X + M1 + M2, data = data_lm_serial)
...
Include `X` and every earlier mediator in each downstream model, as the SEM
version below does. Only `M2`'s coefficient becomes the `b` path, but if `M1`
also affects `Y` directly, an outcome model without `M1` (`Y ~ X + M2`)
confounds it: `M1` is a common cause of `M2` and `Y`, so `b` is biased. The
product `a * d1 * b` is then the effect through the full chain
`X -> M1 -> M2 -> Y` only; a direct `M1 -> Y` path is not part of it.
