...
`mediator1 -> outcome` path is not part of it. `nie(type = "total")` adds the
paths that skip a mediator, and `te()` and `pm()` use every path, so here the
chain carries about 0.08 of a 0.37 total indirect effect.
...
`cov(d_i, b)`, and `cov(d_i, d_j)` are stored as zero. For least squares this
is exact when each downstream equation contains every regressor of the
earlier ones, as recommended above. (The within-outcome-equation covariance,
e.g. `cov(b, c')`, is preserved.)

A single lavaan `sem()` fit of the *same equations* estimates them jointly by
maximum likelihood. Its covariances among chain paths are then also zero, so
the two engines' intervals differ only by the small-sample difference between
the OLS and ML variance estimates. They can differ more when the lavaan model
specifies different equations (for example, omits a path that skips a
mediator) or adds residual covariances.
:::
...
`tidy()` gives the effects with delta-method standard errors and normal
intervals, and `glance()` gives a one-row summary:

```{r}
#| label: lm-parallel-tidy
library(generics)
...
The normal interval for `nie` treats a sum of coefficient products as normal;
for inference on the indirect effect prefer
[`bootstrap_mediation()`](bootstrap.html).
...
`decompose()` returns the controlled direct effect with the joint natural
effects. The CDE is read at the reference level `m_star` of each interacting
mediator (default 0); the NDE, NIE and total effect do not depend on it:

```{r}
#| label: lm-joint-decompose
decompose(med_joint)
#>       cde       nde       nie     total
...
The CDE moves by `t3 * (1 - 0) = 0.49`, while the other three stay put. Unlike
the single-mediator four-way decomposition, the joint object has no
INT<sub>ref</sub>/INT<sub>med</sub> split: the interaction is absorbed into
the joint NDE and NIE.
...
`tidy()` reports the four components with `type = "components"` and the
natural effects with `type = "effects"`; `glance()` adds the interaction
coefficient and `m_star`:

```{r}
#| label: lm-interaction-tidy
library(generics)
