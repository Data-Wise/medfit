...
The examples below use a simulated single-mediator data set (X -> M -> Y):
...
quick(result)
#> NIE = 0.164  [0.0885, 0.252] | NDE = 0.293 | PM = 36 %
...
med_data <- extract_mediation(fit_m, model_y = fit_y, treatment = "X", mediator = "M")
...
For serial mediation (X -> M1 -> M2 -> Y), the indirect effect is the product
of the chain's paths, $a \times d \times b$. The parametric and plugin methods
currently accept only a single-mediator `MediationData` object, so bootstrap a
serial chain nonparametrically by refitting all three models:

```{r}
#| label: serial
# Serial mediation data (X -> M1 -> M2 -> Y)
set.seed(456)
serial_data <- data.frame(X = rnorm(n))
serial_data$M1 <- 0.5 * serial_data$X + rnorm(n)
serial_data$M2 <- 0.4 * serial_data$M1 + 0.2 * serial_data$X + rnorm(n)
serial_data$Y <- 0.4 * serial_data$M2 + 0.2 * serial_data$M1 +
  0.2 * serial_data$X + rnorm(n)

# Fit the chain: each model includes all upstream variables
fit_serial <- function(d) {
  extract_mediation(
    lm(M1 ~ X, data = d),
    model_y = lm(Y ~ X + M1 + M2, data = d),
    treatment = "X",
    mediator = c("M1", "M2"),
    mediator_models = list(lm(M2 ~ X + M1, data = d))
  )
}
serial_med <- fit_serial(serial_data)
...
#>   <chr>       <dbl>     <dbl>    <dbl>     <dbl>
#> 1 estimate    0.164    0.0430   0.0885     0.252
