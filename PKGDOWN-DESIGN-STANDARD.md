...
**Version:** 1.0.0 **Last Updated:** 2025-12-20 **Status:** Standard
Reference Document **Scope:** All mediationverse packages (medfit,
...
This document establishes standard design patterns for R package
documentation sites across the mediationverse ecosystem. It ensures: -
...
**Rule**: Accent colors should only be used in package-specific content
(plots, callouts), NOT in navigation or core UI.

------------------------------------------------------------------------

## Custom CSS Template

All packages should include `pkgdown/extra.css` with these ADHD-friendly
enhancements:
...
Additional CSS can be added for: - Package-specific callout boxes (use
accent colors) - Function category icons - Visual cards for feature
grids - Dark mode (if desired)
...
4.  **Ecosystem** - Cross-package navigation (CRITICAL for
    discoverability)
5.  **News** - Changelog
...
**Rule**: Keep menu synchronized across all packages. When a new package
is added, update ALL package sites.
...
**Rationale:** - **Inter**: Clean sans-serif, excellent readability for
long-form content - **Montserrat**: Geometric sans-serif, strong
presence for headings - **Fira Code**: Monospace with ligatures, ideal
for R code
...
Higher line-height (1.7) for body improves readability, especially for
ADHD users.
...
**Problem**: Flat text walls are hard to scan. **Solution**: Strong
...
**Problem**: Long content is overwhelming. **Solution**: Generous
...
**Problem**: Hard to distinguish different content types. **Solution**:
Colored borders for categorization

- Code blocks: Blue left border (R input) vs gray (output)
- Callouts: Yellow (TL;DR), blue (info), green (success), orange
  (warning)
- Links: Blue (primary actions)

### Sticky Navigation

**Problem**: Lost context in long articles. **Solution**: Sticky table
of contents
...
**CRITICAL**: Badges must be OUTSIDE the `<!-- badges: -->` comment
markers for pkgdown Bootstrap 5.
...
**medfit** (Foundation package): - Site:
<https://data-wise.github.io/medfit/> - Features: Full implementation of
Plan A - Status: ✅ Reference implementation

**probmed** (Application package): - Site:
<https://data-wise.github.io/probmed/> - Accent: Purple (#7C3AED) for
probability visualizations - Status: 🚧 Pending redesign

**RMediation** (Application package): - Site:
<https://data-wise.github.io/rmediation/> - Accent: Teal (#17A2B8) for
CI highlights - Status: 🚧 Pending redesign

------------------------------------------------------------------------

## Future Enhancements (Plan B)

The following enhancements may be added in Phase 2 (Plan B
implementation):
...
**For questions about this standard:** - Open issue:
<https://github.com/data-wise/medfit/issues> - Discuss in ecosystem
planning: `/Users/dt/mediation-planning/`

**For implementation help:** - See PROPOSAL-SITE-REDESIGN.md for
detailed examples - Check medfit implementation:
<https://github.com/data-wise/medfit> - Review pkgdown documentation:
...
**Document Status**: ✅ Active Standard **Next Review**: After Plan B
implementation in any package **Maintained By**: mediationverse
development team
